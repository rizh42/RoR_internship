...
class Interface
  def initialize
    puts "Available comands:\n " \
         ...
         'm) Get all information about all trains\n' \
         "ENTER) Quit\n" \
         'Enter command:'
  ...
  def create_train
    puts "Enter train's id, type and volume of seats or cargo available:"
    tmp = gets.chomp!.split(' ')
    if tmp[1] == 'cargo'
      trains[tmp[0]] = CargoTrain.new(tmp[0], tmp[2].to_i)
      puts " Train's cargo volume: #{tmp[2].to_i}"
    else
      trains[tmp[0]] = PassengerTrain.new(tmp[0], tmp[2].to_i)
      puts " Train's passenger seats: #{tmp[2].to_i}"
    end
  ...
  def create_route
    if stations.size > 1
      ...
      puts "Enter first and last stations' name"
      tmp = gets.chomp!.split(' ')
  ...
  def add_wagon_to_train
    ...
    tmp = gets.chomp!.split(' ')
    trains[tmp[1]].add_wagon(Wagon.new(tmp[0]), trains.tmp[1].wagons.size)
  end
  ...
  def trains_at_station
    ...
    tmp = gets.chomp!
    stations[tmp].trains
    stations[tmp].trains.wagons do |wagon|
      puts ' All wagons:'
      puts "  #{wagon.number}: free volume: #{wagon.free_volume}, occupied volume: #{wagon.volume}"
    end
  end
  ...
  def all_trains
    stations.each do |station|
      station.trains.each do |train|
        puts "Train's ID: #{train.id}, type: #{train.type}, wagons' count: #{train.wagons.size}"
      end
    end
  end
end
